...
const sketch = () => {
  return ({ context, width, height }) => {
    ...
    // Gradient foreground
    const fill = context.createLinearGradient(0, 0, width*2, height);
    ...
    for(var i=0; i<outerLoop; i++){ 
      ...
      createCircle(width, height, context, i/4)
      for(var j=0; j<innerLoop; j++){
        ...
        createCircle(width/2, height, context, j/32)//j/64)
      }
    }
...
const createCircle = (width, height, context, radius) => {
  ...
  // context.beginPath();
  // context.arc(getRandomNumber(width-borderMargin, radius/4*2), getRandomNumber(height-borderMargin, radius*2), radius, 0, 2 * Math.PI, false);
  // context.fillStyle = "rgba(255,255,255,0.1)"
  // context.fill();
